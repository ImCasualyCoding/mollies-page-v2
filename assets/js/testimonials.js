/* ==========================================================================
   MoveRx Testimonials Webpage Logic (v2)
   Description: Dynamic JS content injection, navigation rendering, testimonials
                grid construction, dynamic fling success story carousel,
                and green footer rendering.
   ========================================================================== */

/* 1. CENTRALIZED TESTIMONIALS PAGE CONTENT DEFINITIONS
   ==========================================================================
...
const testimonialsContent = {
        ...
        stories: [
            {
                ...
                title: "Posture Alignment & Core Stability",
                clientName: "Client A",
                description: "Biomechanical program targeting posture alignment, shoulder flexibility, and core power. Full details will be provided later.",
                beforeImage: "assets/images/misc/placeholder-before.png",
                afterImage: "assets/images/misc/placeholder-after.png"
            },
            {
                category: "Joint Rehabilitation",
                title: "Functional Post-Surgery Mobility",
                clientName: "Client B",
                description: "Specialized post-surgery mobility and balance stabilization program to restore gait independence. Full details will be provided later.",
                beforeImage: "assets/images/misc/placeholder-before.png",
                afterImage: "assets/images/misc/placeholder-after.png"
            },
            {
                category: "Muscle Sculpting",
                title: "Muscle Sculpting & Resiliency",
                clientName: "Client C",
                description: "Low-impact resistance training program building lean muscle mass and metabolic stamina. Full details will be provided later.",
                beforeImage: "assets/images/misc/placeholder-before.png",
                afterImage: "assets/images/misc/placeholder-after.png"
            },
            {
                category: "Elderly Mobility",
                title: "Coordination & Fall Prevention",
                clientName: "Client D",
                description: "Coordination training designed to improve range of motion, prevent falls, and build confidence. Full details will be provided later.",
                beforeImage: "assets/images/misc/placeholder-before.png",
                afterImage: "assets/images/misc/placeholder-after.png"
            }
        ]
    ...
    // Vector graphic SVGs used as decorative scroll-animated elements
    svgs: {
        monstera: `
...
function renderNavigation() {
    ...
    logoLink.href = testimonialsContent.navigation.logo.link;
    logoLink.className = "logo";
    logoLink.ariaLabel = `${testimonialsContent.navigation.logo.textHighlight}${testimonialsContent.navigation.logo.textRegular} Home`;
    logoLink.innerHTML = `
        <span class="logo-move">${testimonialsContent.navigation.logo.textHighlight}</span><span class="logo-rx">${testimonialsContent.navigation.logo.textRegular}</span>
    `;
    ...
    testimonialsContent.navigation.links.forEach(item => {
    ...
    // Mobile-only consultation booking button
    const mobileCtaLi = document.createElement("li");
    mobileCtaLi.className = "mobile-only";
    const mobileCta = document.createElement("a");
    mobileCta.href = testimonialsContent.navigation.cta.url;
    mobileCta.className = "btn-primary";
    mobileCta.textContent = testimonialsContent.navigation.cta.text;
    ...
    desktopCta.href = testimonialsContent.navigation.cta.url;
    desktopCta.className = "btn-primary-small";
    desktopCta.textContent = testimonialsContent.navigation.cta.text;
...
/**
 * Renders the Testimonials Hero Section
 */
function renderHero() {
    ...
    heroSection.className = "testimonial-hero";
    ...
    container.className = "container testimonial-hero-content animate-fade-in";

    const heading = document.createElement("h1");
    heading.textContent = testimonialsContent.hero.title;
    container.appendChild(heading);

    const subtext = document.createElement("p");
    subtext.className = "hero-subtext";
    subtext.textContent = testimonialsContent.hero.subtext;
...
/**
 * Renders the Client Success Stories Showcase Carousel (Fling swap)
 * Note: Heading and description are removed per request.
 * Displays only a singular item at a time.
 */
function renderSuccessStories() {
    const mainContent = document.getElementById("main-content");
    if (!mainContent) return;

    const storiesData = testimonialsContent.successStories;

    const successSection = document.createElement("section");
    successSection.className = "success-section";
    successSection.id = "success";
    successSection.style.padding = "40px 0 60px";

    const container = document.createElement("div");
    container.className = "container";

    // Showcase Fling Wrapper
    ...
    function renderCard(index, animationClass = "") {
        ...
        transitionBox.setAttribute("aria-label", `After progress photos for ${story.title}. Tap to toggle prior photo.`);

        const imgBefore = document.createElement("img");
        imgBefore.src = story.beforeImage;
        imgBefore.alt = `${story.title} - Before State`;
        imgBefore.className = "img-before";

        const imgAfter = document.createElement("img");
        imgAfter.src = story.afterImage;
        imgAfter.alt = `${story.title} - After Progress State`;
        ...
        // Click to toggle prior photo (show-before)
        const togglePhoto = () => {
    ...
    // Carousel Navigation with fling transitions
...
/**
 * Renders the grid of client testimonials (the John C., Cathy B., etc. reviews)
 */
function renderTestimonialsGrid() {
    ...
    testimonialsContent.testimonials.forEach((item, index) => {
        ...
        starsRow.className = "testimonial-stars";
        starsRow.setAttribute("aria-label", `${item.stars} star rating`);
        for (let i = 0; i < item.stars; i++) {
            starsRow.innerHTML += `
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 2px;">
...
function renderLeaves() {
    ...
    testimonialsContent.leaves.forEach(leafData => {
        ...
        leafContainer.innerHTML = testimonialsContent.svgs[leafData.svgType];
    ...
    function updateLeaves() {
        ...
        leaves.forEach(leaf => {
            ...
            if (distance < maxDistance) {
                ...
                const ease = Math.sin(progress * Math.PI / 2); // Sine ease-out sways
